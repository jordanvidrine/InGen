...
async function getSectionContent() {
  ...
  try {
    ...
    for (let section of sections) {
      ...
      if (/(.md)$/.test(section)) {
        let rawContent = await fs.readFileSync(
...
async function getPosts() {
  ...
  try {
    ...
    for (let post of posts) {
      let postContent = await fs.readFileSync(
        `./content/posts/${post}`,
        "utf8"
      );

      // parse options and store to an object
      let dataArray = postContent
        .split("\n")
        .map((line) => line.replace("\r", ""));
      let postData = {};
      dataArray
        .splice(dataArray.indexOf("---") + 1, dataArray.lastIndexOf("---") - 1)
        .forEach((data) => {
...
async function getPageContent(path) {
  ...
  contentArray
    .splice(
      contentArray.indexOf("---") + 1,
      contentArray.lastIndexOf("---") - 1
    )
    .forEach((option) => {
